...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed()*KmHInMsec, 2)/(w.Height/float64(CmInM)))*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration.Hours()) * float64(MinInHours))
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	TrainingInfoMessage.Distance = s.meanSpeed() * (s.Duration.Hours()) //дистанция в метрах
	return TrainingInfoMessage
}
